...
function run2ndScreenPt2() {
  document.querySelector(
    ".video_content"
  ).innerHTML += `<div class="logo">${logo}</div>`;
  document
    .querySelector(".video_content")
    .addEventListener("animationend", function _function() {
      setTimeout(run3rdScreen, 2000);
      document
        .querySelector(".video_content")
        .removeEventListener("animationend", _function);
    });
}
...
function run4thScreen() {
  ...
  document.querySelector(
    ".character_container"
  ).innerHTML = `<div class="character camilla">${camilla}</div>`;
  document
...
function run6thScreen() {
  ...
  document.querySelector(
    ".character_container"
  ).innerHTML = `<div class="character julie">${julie}</div>`;
  document
    .querySelector(".character")
    .addEventListener("animationend", function() {
      document.querySelector(".text_content").classList.add("character_text");
...
function run7thScreen() {
  ...
  document.querySelector(
    ".character_container"
  ).innerHTML = `<div class="character julie">${julie}</div> <div class="character camilla">${camilla}</div>`;
  document
    .querySelector(".character")
    .addEventListener("animationend", function() {
      document.querySelector(".text_content").classList.add("character_text");
...
function typewriterEffect(text, nextFunction, destination, delay) {
  ...
  if (index <= text.length) {
    ...
    index = 0;
    setTimeout(nextFunction && nextFunction, delay);
  }
}
